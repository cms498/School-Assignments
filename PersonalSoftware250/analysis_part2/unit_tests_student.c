...
int test_student()
{
	...
	assert(entry.word_count == 3, "2: the word day must appear 3 times in the list but it appeard %d\n", entry.word_count)
		? passcount++ : failcount++;
	// Add a test to verify that the word AFTER the word "have" is the word "heat".
	struct word_entry entry2;
	find_word(&list1, "have");
	entry2 = get_next_word(&list1);
	assert(strcmp(entry2.unique_word, "heat") == 0, "3: word after have must be heat but it got %s\n", entry2.unique_word)
		? passcount++ : failcount++;
	// Add a test to verify that the last word in the list is "with"
	struct word_entry entry3;
	entry3 = get_last_word(&list1);
	assert(strcmp(entry3.unique_word, "with") == 0, "4: last word should be with but got %s\n", entry3.unique_word)
		? passcount++ : failcount++;
	// Add a test to verify that the next to the last word is "will"	
	entry3 = get_prev_word(&list1);
	assert(strcmp(entry3.unique_word, "will") == 0, "5: the last words previous should be will but it got %s\n", entry3.unique_word)
		? passcount++ : failcount++;
