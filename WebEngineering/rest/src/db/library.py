...
def select_books_table():
    ...
    """
    return exec_get_all('SELECT books.id, title, book_type, publish_date, author, copies, summary, library_name FROM books inner join libraries on libraries.id = books.library_id')
...
def list_user_information(username):
    return exec_get_all("SELECT username, email, phone from users where username = %s", (username,))
...
def list_all_books_by_type(book_type):
    ...
    """
    sql = 'SELECT books.id, title, book_type, publish_date, author, copies, summary, library_name FROM books inner join libraries on libraries.id = books.library_id WHERE books.book_type = %s;'
    result = exec_get_all(sql, (book_type,))
    return result;

def list_all_books_by_author(author):
    sql = 'SELECT books.id, title, book_type, publish_date, author, copies, summary, library_name FROM books inner join libraries on libraries.id = books.library_id where books.author = %s'
    return exec_get_all(sql, (author,))

def list_all_books_by_criteria(type, author, title, library):
    sql = 'SELECT books.id, title, book_type, publish_date, author, copies, summary, library_name FROM books inner join libraries on libraries.id = books.library_id where books.book_type like %s and books.author like %s and books.title like %s and library_name like %s'
    args = (type, author, title, library)
...
def create_new_user(name, email, phone, password):
    ...
    """
    sql_find = 'SELECT * FROM users WHERE email = %s and phone = %s and username = %s'
    sql_find_args = (email, phone, name)
    find_result = exec_get_one(sql_find, sql_find_args)
    if find_result == None:
        hashed_password = hashlib.sha512(bytes(password, 'ascii')).hexdigest()
        session_key = binascii.hexlify(os.urandom(24)) 
        sql = 'INSERT INTO users (username, email, phone, hashed_password, session_key) VALUES (%s, %s, %s, %s, %s);'
        args = (name, email, phone, hashed_password, session_key)
...
def list_users_hashed_password_and_session_key(username):
    return exec_get_all("SELECT hashed_password, session_key from users where username = %s", (username,))

def update_user_info(oldemail, newname, newemail, newphone, newpassword):
    sql_find = 'SELECT * FROM users WHERE email = %s'
    sql_find_args = (oldemail,)
    find_result = exec_get_one(sql_find, sql_find_args)
    if find_result != None:
        hashed_password = hashlib.sha512(bytes(newpassword, 'ascii')).hexdigest()
        sql = 'UPDATE users set username = %s, email = %s, phone = %s, hashed_password = %s where email = %s'
        args = (newname, newemail, newphone, hashed_password, oldemail)
        exec_commit(sql, args)
        result = exec_get_one("SELECT * FROM users WHERE email = %s", (newemail,))
        return result
    else:
        return None

def update_session_key(name, email):
    new_key = binascii.hexlify(os.urandom(24)) 
    sql = 'UPDATE users SET session_key = %s WHERE username = %s and email = %s'
    args = (new_key, name, email)
    exec_commit(sql, args)
    return new_key
...
def delete_user(username):
    ...
    """
    sql = "DELETE FROM users where users.username = %s"
    exec_commit(sql, (username,))
    result = exec_get_one('SELECT (username, email, phone) FROM users WHERE users.username = %s', (username,))
    return result
...
def check_out_book(book_name, username, checked_out_date):
    ...
    return exec_get_all("SELECT id, books_id, checked_out_by, checked_out_date, returned_date, due_date, remaining_copies FROM checked_out WHERE books_id = %s AND checked_out_by = %s AND checked_out_date = %s", (book_id, username, checked_out_date))
...
def search_for_book(book_name):
    ...
    """
    return exec_get_all("SELECT books.id, title, book_type, publish_date, author, copies, summary, library_name FROM books inner join libraries on libraries.id = books.library_id WHERE title = %s", (book_name,))
...
def list_all_avaliable_books_by_library(library_name):
    ...
    """
    sql = 'SELECT books.id, title, book_type, publish_date, author, copies, summary, library_name FROM books INNER JOIN libraries on books.library_id = libraries.id WHERE library_name = %s AND books.copies > 0'
    result = exec_get_all(sql, (library_name,))
    return result
